...
const ADMIN_SESSION_KEY = "cashAppWorkflowTool.adminUnlocked";
...
function loadWorkflowData() {
  ...
  if (window.MISDIRECTED_PAYMENT_WORKFLOW) {
...
function initializeAdminAccess() {
  ...
  const input = document.querySelector("#adminPasscodeInput");
  const error = document.querySelector("#adminAccessError");
  ...
  form.addEventListener("submit", async (event) => {
    ...
    const candidateHash = await hashText(input.value);
    const expectedHash = window.ACCESS_CONFIG?.adminPasscodeHash || "";
    if (candidateHash !== expectedHash) {
      error.textContent = "Incorrect admin passcode.";
      input.select();
      return;
    }
...
function openAdminAccessDialog() {
  const dialog = document.querySelector("#adminAccessDialog");
  document.querySelector("#adminAccessError").textContent = "";
  document.querySelector("#adminPasscodeInput").value = "";
  if (!dialog.open) dialog.showModal();
  requestAnimationFrame(() => document.querySelector("#adminPasscodeInput").focus());
}
